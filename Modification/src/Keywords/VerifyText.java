...
public String testing(Properties p,String[] record,int row, String sh, int resultRow,String[] imp ) 
	...
	try {
		...
		for(int i=0;i<onames.length;i++){
			try{
		WebElement verifyPass=obj.getFluent().fluentWait(obj.getLocators().getObject(p,onames[i],otypes[i]));
		...
		if(count==onames.length){
			...
			obj.getResultScreenshot().demand(record[STEPNUMBER],"Web page is showing "+data,Information.PASS,imp);
		}
		else{
			cond= false;
			obj.getExcelResult().setData(cond,p,row,sh,resultRow,Information.FAIL,imp);
			obj.getResultScreenshot().demand(record[STEPNUMBER],"Web page is showing "+data,Information.FAIL,imp);
		}
		for(int j=0;j<onames.length;j++){
			try{
				if(obj.getVisible().isElementPresent(obj.getLocators().getObject(p,onames[j],otypes[j]))){
			WebElement verify=obj.getFluent().fluentWait(obj.getLocators().getObject(p,onames[j],otypes[j]));
...
public String negative(Properties p,String[]  record,int row, String sh, int resultRow,String[] imp ) throws Exception
{
	try{
		...
		if(obj.getVisible().isElementPresent(neg)){
			...
			obj.getExcelResult().setData(false,p,row,sh,resultRow,Information.FAIL,imp);
			obj.getJavaScript().mark(negative);
			obj.getResultScreenshot().demand(record[STEPNUMBER],"Web page is showing "+negmsg,Information.FAIL,imp);
...
public String inputVerify(Properties p,String[]  record,int row, String sh, int resultRow,String[] imp ) throws Exception{
	try {
		WebElement element = obj.getFluent().fluent(obj.getLocators().getObject(p, record[OBJECTNAME], record[OBJECTTYPE]));
		...
		if(elementText.equals(record[VALUE])) {
			cond = true;
			obj.getExtentTest().log(LogStatus.PASS, record[STEPNUMBER], record[VALUE]+" matches with expected value "+elementText);
			obj.getExcelResult().setData(cond,p,row,sh,resultRow,Information.PASS,imp);
			return Information.PASS;
		}else {
			cond = false;
			obj.getExtentTest().log(LogStatus.FAIL, record[STEPNUMBER], record[VALUE]+" is not matching with expected value "+elementText);
			obj.getExcelResult().setData(cond,p,row,sh,resultRow,Information.FAIL,imp);
			return Information.FAIL;
		}
	} catch (Exception e) {
...
public String dropdownVerify(Properties p,String[]  record,int row, String sh, int resultRow,String[] imp ) throws Exception{
	try {
		WebElement element = obj.getFluent().fluent(obj.getLocators().getObject(p, record[OBJECTNAME], record[OBJECTTYPE]));
		...
		if(elementText.equals(record[VALUE])) {
			cond = true;
			obj.getExtentTest().log(LogStatus.PASS, record[STEPNUMBER], record[VALUE]+" matches with expected value "+elementText+" in dropdown");
			obj.getExcelResult().setData(cond,p,row,sh,resultRow,Information.PASS,imp);
			return Information.PASS;
		}else {
			cond = false;
			obj.getExtentTest().log(LogStatus.FAIL, record[STEPNUMBER], record[VALUE]+" is not matching with expected value "+elementText+" in dropdown");
			obj.getExcelResult().setData(cond,p,row,sh,resultRow,Information.FAIL,imp);
			return Information.FAIL;
		}
	} catch (Exception e) {
